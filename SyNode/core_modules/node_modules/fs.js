...
fs.exists = function(path, callback) {
  ...
  let err = null;
  try {
    binding.fileStat(pathModule._makeLong(path));
  } catch (e) {
    err = e
  }
  cb(err);
...
fs.existsSync = function (path){
  try {
    handleError((path = getPathFromURL(path)));
    nullCheck(path);
    return !!binding.fileStat(pathModule._makeLong(path));
  } catch (e) {
    return false;
  }
};
...
fs.readFile = function(path, options, callback) {
  ...
  let res
  try {
    res = fs.readFileSync(path, options)
  } catch (e) {
    err = e
  }
  callback(err, res)
...
fs.open = function(path, flags, mode, callback_) {
  ...
  let res;
  try {
    res = binding.open(pathModule._makeLong(path), stringToFlags(flags), mode)
  } catch (e) {
    err = e
  }
  oncomplete(err, res)
...
fs.read = function(fd, buffer, offset, length, position, callback) {
  ...
  let res
  try {
    res = binding.read(fd, buffer, offset, length, /*position*/null);
  } catch (e) {
    err = e
  }
  oncomplete(err, res)
...
fs.readSync = function(fd, buffer, offset, length, position) {
  ...
  return binding.read(fd, buffer, offset, length, /*position*/null);
};
...
fs.write = function(fd, buffer, offset, length, position, callback) {
  ...
  if (isUint8Array(buffer)) {
    ...
    // if (typeof position !== 'number') { -- not null position is not currently supported
    //  position = null;
    // }
    try {
      written = binding.writeBuffer(fd, buffer, offset, length, null/*position*/);
    } catch (e) {
      err = e
    }
...
fs.readdir = function(path, options, callback) {
  ...
  let res
  try {
    res = binding.readdir(pathModule._makeLong(path), options.encoding);
  } catch (e) {
    err = e
  }
  oncomplete(err, res)
...
fs.lstat = function(path, callback) {
  ...
  let res;
  try {
    res = binding.fileStat(pathModule._makeLong(path), true);
  } catch (e) {
    err = e
  }
  oncomplete(err, res);
...
fs.fchmod = function(fd, mode, callback) {
  ...
  let res;
  try {
    res = binding.chmodFile(fd, modeNum(mode));
  } catch (e) {
    err = e;
  }
  oncomplete(err, res);
...
fs.fchmodSync = function(fd, mode) {
  return binding.chmodFile(fd, modeNum(mode));
};
...
fs.chmod = function(path, mode, callback) {
  ...
  let res;
  try {
    res = binding.chmodFile(pathModule._makeLong(path), modeNum(mode));
  } catch (e) {
    err = e
  }
  oncomplete(err, res);
...
fs.chmodSync = function(path, mode) {
  handleError((path = getPathFromURL(path)));
  nullCheck(path);
  return binding.chmodFile(pathModule._makeLong(path), modeNum(mode));
};
...
fs.writeFile = function(path, data, options, callback) {
  ...
  function writeFd(fd, isUserFd) {
    ...
    if (!isUint8Array(data) && typeof data === 'object') {
      try {
        written = binding.writeFile(fd, data); // ub compatibility workaround
      } catch (e) {
        err = e
      }
      if (!isUserFd) {
      ...
      var position = null // not currently supported: /a/.test(flag) ? null : 0;
      try {
        written = fs.writeSync(fd, buffer, 0, buffer.length, position)
      } catch (e) {
        err = e
      }
      if (!isUserFd) {
...
/*******************************
 * Deprecated functions - implemented for compatibility with previous versions on UB
 */
