...
//SyNode const { AsyncWrap, Providers } = internalBinding('async_wrap');
...
function pbkdf2(password, salt, iterations, keylen, digest, callback) {
  ...
  if (!callback) return handleError(keybuf, password, salt, iterations, digest);
  const rc = _pbkdf2(keybuf, password, salt, iterations, digest);
  if (rc === -1) {
    cb(new ERR_CRYPTO_INVALID_DIGEST(digest));
  } else if (rc === false) {
    return cb(new ERR_CRYPTO_PBKDF2_ERROR());
  } else {
    cb(null, keybuf)
  }
}
