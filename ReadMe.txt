...
as a server, or a client, depending on some run-time parameters!
...
up to the most complex Domain-Driven Design (DDD):
 ...
 communication protocols (including HTTP/1.1);
 ...
 for Value Objects, or dedicated classes for Entities or Aggregates;

 - Data persistence infrastructure layer with ORM persistence over direct
 Oracle, MS SQL, OleDB, ODBC, ZEOS/ZDBC access or TDataSet libraries like
 FireDAC/AnyDAC, UniDAC, NexusDB or BDE, with a powerful SQLite3 kernel,
 and direct SQL access if needed - with amazing performance and advanced
 features like Array DML - auto-generating SQL for SQLite3, Oracle, 
 Jet/MSAccess, MS SQL, Firebird, DB2, PostgreSQL, MySQL and NexusDB;

 - Cross-Cutting infrastructure layers for handling data filtering and
 validation, security (even Windows authentication or any custom model),
 caching, logging and testing (framework uses test-driven approach and
 includes interface stubbing and mocking).

With mORMot, ORM is not used only for data persistence of objects (like
...
screen auto-generation, reporting and ribbon GUI), and you can use it from any
RAD or AJAX clients (via JavaScript or even the newborn Smart Mobile Studio).
...
encrypted via HTTS or directly with SHA/AES-256. Endpoints are configured
automatically for each published interface on the server side, and creating
a load-balancing proxy is a matter of one method call.
...
even creating the server side part of an AJAX application.
...
Issues and feature requests can be posted (but take a look at the forum
...
of the supplied SAD document. It will cover all key-concepts and code modeling
...
Some units (like SynPdf, SynGdiPlus, SynBigTable, SynCommons, SynDB*,
SynSQLite3, mORMotReport) are used by mORMot, but do not require the
whole framework to be used.
That is, you can use e.g. only the PDF generation features of SynPdf,
the fast database access classes of SynDB, the static-linked SQLite3
engine of SynSQLite3Static, the code-generated reports of mORMotReport,
or the TDynArray / TSynLog classes of SynCommons, without using the main
mORMot classes and features (ORM, Client-Server, services, UI, reporting).
...
4) Change your references to mORMot units:
 ...
 - Add in your uses clauses SynSQLite3Static unit (for Win32).
...
5) Consult the units headers about 1.18 for breaking changes, mainly:
 ...
 - rmJSON* enums replaced by TSQLRestRoutingREST and TSQLRestRoutingJSON_RPC classes;
