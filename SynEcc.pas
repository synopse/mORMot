...
type
  ...
  /// result of TECCSignatureCertified.Verify and TECCCertificateChain.IsValid methods
  // - and low-level ECCVerify() function
...
type
  ...
  protected
    ...
    function InternalAdd(cert: TECCCertificate; expected: TECCCertificateValidity): integer;
  ...
  public
    ...
    // - this method is thread-safe, and not blocking
    function IsValid(cert: TECCCertificate): TECCCertificateValidity; overload;
    /// check if the certificate is valid, against known certificates chain
    // - will check internal properties of the certificate (e.g. validity dates,
    ...
    // - supplied hash is likely to be from SHA-256, but could be e.g. crc256c
    // - this method is thread-safe, and not blocking
    function IsSigned(sign: TECCSignatureCertified; const hash: THash256): TECCCertificateValidity; overload;
    ...
    // stored in the .sign file content will be checked before ECDSA signature,
    // and would return ecvCorrupted
    // - will compute and verify the SHA-256 hash of the supplied data
    // - it will then check internal properties of the certificate (e.g. validity
    // dates), and validate the stored ECDSA signature according to the public
    // key of the associated signing authority (stored in Items[])
    // - this method is thread-safe, and not blocking
...
function ToText(val: TECCCertificateValidity): PShortString;
begin
...
function TECCCertificateChain.IsValid(cert: TECCCertificate): TECCCertificateValidity;
begin
...
function TECCCertificateChain.IsValid(const content: TECCCertificateContent;
  ignoreDate: boolean): TECCCertificateValidity;
...
function TECCCertificateChain.InternalAdd(cert: TECCCertificate;
  expected: TECCCertificateValidity): integer;
begin
  result := -1;
