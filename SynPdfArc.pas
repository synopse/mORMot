...
type
  ...
  *)
function doArc(centerx, centery, W, H, Sx, Sy, Ex, Ey: integer; aClockWise: boolean; isPieSlice: Boolean; var res:
...
const
  ...
  // 0 < b/a < 1/4
  coeffs3Low: tcoeffArray = (((3.85268, -21.229, -0.330434, 0.0127842), (-1.61486, 0.706564, 0.225945, 0.263682),
  ...
  // while using cubic Bézier curves for approximation
  // 1/4 <= b/a <= 1
  coeffs3High: tcoeffArray = (((0.0899116, -19.2349, -4.11711, 0.183362), (0.138148, -1.45804, 1.32044, 1.38474),
...
type
  ...
  public
    constructor CreateArcI(clockwise: Boolean; centerx, centery, W, H, Sx, Sy, Ex, Ey: integer; PieSlice: Boolean);
...
begin
  ...
  size := n + 1; // Res Size n+1 because first move
  if fisPieSlice then
    inc(size, 2); // for first and last Line if wie want a pie

  setlength(res, size);
  resindex := 0;

  if fisPieSlice then begin
...
constructor tCalcArc.CreateArcI(clockwise: Boolean; centerx, centery, W, H, Sx, Sy, Ex, Ey: integer; PieSlice:
  Boolean);
begin
  inherited create;
  fcx := centerx;
  ...
  faRad := (W) / 2;
  fbRad := H / 2;
  fisPieSlice := PieSlice;
  // Calculate Rotation at Start and EndPoint
  if clockwise then begin
    feta1 := ArcTan2(Sy - fcy, Sx - fcx);
    feta2 := ArcTan2(Ey - fcy, Ex - fcx);
  end else begin
    feta2 := ArcTan2(Sy - fcy, Sx - fcx);
    feta1 := ArcTan2(Ey - fcy, Ex - fcx);
  end;
...
begin
  setlength(res, 0);
  test := tCalcArc.CreateArcI(aClockWise, centerx, centery, W, H, Sx, Sy, Ex, Ey, isPieSlice);
